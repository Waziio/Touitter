...
if (isset($_SESSION['connexion'])) {
    // posts : affichage des posts, q : valeur entrée dans la barre de recherche de users
    if ((isset($_GET['posts'])) || (isset($_GET['q']))) {
        try {
            $db = new PDO("$server:host=$host;dbname=$base", $user, $pass);
            // on recup tout les posts triés par date et heure, minute ET secondes
            $sql = "SELECT id_post, msg_post, id_user, DATE_FORMAT(date_post, '%d/%m/%Y') AS 'date', TIME_FORMAT(heure_post, '%H:%i') AS 'heure' FROM Post WHERE public_post = 1 ORDER BY date_post DESC, heure_post DESC;";
            $st = $db->query($sql);
            $lignes = $st->fetchAll();
...
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge" on>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Touitter - Fil Principal</title>
...
<body>
    ...
    <div id="listeTouitte">
        <h2 id=titlePerso>Fil Principal</h2>
        <?php
        // Si ily'a des posts à afficher
        if (!empty($lignes)) {
            ...
            foreach ($lignes as $key => $value) {
                ...
                // On affiche le nombre d'étoiles en fcontion de la note moy du post
                switch ($noteMoyenne) {
                    case 1:
            ...
            // Si il n'ya aucun post à afficher
